...
import {
  SPS_DATE_FILTER_KEYS,
  SPS_DROPDOWN_FILTER_KEYS,
  SPS_FILTER_LABELS,
  SPS_MONTH_FILTER_KEYS,
} from '@/lib/sps/constants';
import type { SpsFilterKey } from '@/lib/sps/types';
import { useSps } from '@/providers/SpsProvider';

export default function SpsFilterPanel() {
  const { filters, setFilter, resetFilters, filterOptions, loading, isFetching } = useSps();
  const busy = loading || isFetching;

  const renderDateFilter = (key: SpsFilterKey) => (
    <FilterDateRange
      key={key}
      label={SPS_FILTER_LABELS[key]}
      value={filters[key]}
      onChange={(v) => setFilter(key, v)}
      mode={SPS_MONTH_FILTER_KEYS.includes(key) ? 'month' : 'date'}
      loading={busy}
    />
  );
    ...
    <aside className="h-full w-56 shrink-0">
      <div className="glass-panel flex h-full flex-col overflow-hidden shadow-2xl shadow-black/20">
        ...
        <div className="flex-1 space-y-2.5 overflow-y-auto p-2 custom-scrollbar">
          {SPS_DROPDOWN_FILTER_KEYS.map((key) => (
            <FilterDropdown
              key={key}
              label={SPS_FILTER_LABELS[key]}
              value={filters[key]}
          ...
          <div className="border-t border-slate-800/80 pt-2">
            ...
            <div className="space-y-2.5">
              {SPS_DATE_FILTER_KEYS.map((key) => renderDateFilter(key))}
            </div>
          </div>
        </div>

...
import { KEHE_FILTER_KEYS, KEHE_FILTER_LABELS } from '@/lib/stores/kehe/constants';
import type { KeheFilterKey } from '@/lib/stores/kehe/types';
import { useKehe } from '@/providers/KeheProvider';

export default function KeheFilterPanel() {
  const { filters, setFilter, resetFilters, filterOptions, loading, isFetching } = useKehe();
    ...
    <aside className="h-full w-56 shrink-0">
      <div className="glass-panel flex h-full flex-col overflow-hidden shadow-2xl shadow-black/20">
        ...
        <div className="flex-1 space-y-2.5 overflow-y-auto p-2 custom-scrollbar">
          {KEHE_FILTER_KEYS.map((key: KeheFilterKey) => (
            <FilterDropdown
              key={key}
              label={KEHE_FILTER_LABELS[key]}
              value={filters[key]}

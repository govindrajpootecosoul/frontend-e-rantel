...
export default function KeheRiskStockStatusTable({
  data,
  loading,
}: {
    ...
    <div className="glass-panel overflow-hidden">
      <div className="border-b border-slate-800 bg-slate-900/80 px-4 py-3">
        ...
        <p className="mt-0.5 text-[10px] text-slate-500">
          Only for SKU + DC from your risk file — upload risk data first, then KeHE Inventory tab
          for stock details
        </p>
      </div>
              ...
              <tr>
                <td colSpan={9} className="px-4 py-10 text-center text-slate-500">
                  {data?.hasRiskData === false
                    ...
                    : 'No matching rows in KeHE Inventory for these risk SKU/DC — upload inventory on the KeHE Inventory tab'}
                </td>
              </tr>
            ) : (
              rows.map((row, i) => (

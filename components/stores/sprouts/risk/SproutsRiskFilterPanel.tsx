...
import { SPROUTS_RISK_FILTER_KEYS, SPROUTS_RISK_FILTER_LABELS } from '@/lib/stores/sprouts/constants';
import type { SproutsRiskFilterKey } from '@/lib/stores/sprouts/risk-types';
import { useSprouts } from '@/providers/SproutsProvider';

export default function SproutsRiskFilterPanel() {
  const {
    riskFilters,
    setRiskFilter,
    resetRiskFilters,
    ...
    <aside className="h-full w-56 shrink-0">
      <div className="glass-panel flex h-full flex-col overflow-hidden shadow-2xl shadow-black/20">
        ...
        <div className="flex-1 space-y-2.5 overflow-y-auto p-2 custom-scrollbar">
          {SPROUTS_RISK_FILTER_KEYS.map((key: SproutsRiskFilterKey) => (
            <FilterDropdown
              key={key}
              label={SPROUTS_RISK_FILTER_LABELS[key]}

...
export default function SproutsInventorySkuGrid({
  data,
  loading,
}: {
  ...
  const rows = useMemo(() => {
    const map = new Map<
      string,
      {
        ...
        byDc: Map<string, Record<SproutsAgingBucketId, number>>;
      }
    >();
    ...
    for (const item of data?.skuGrid ?? []) {
      ...
      if (!entry.byDc.has(item.dc)) {
        entry.byDc.set(
          item.dc,
          Object.fromEntries(buckets.map((b) => [b.id, 0])) as Record<SproutsAgingBucketId, number>
        );
      }
                    ...
                    return (
                      <Fragment key={`${row.sku}-${dc}`}>
                        {buckets.map((b) => (
                          ...
                          >
                            {formatNumber(
                              dcBuckets?.[b.id as SproutsAgingBucketId] ?? 0
                            )}
                          </td>
                        ))}
                      </Fragment>

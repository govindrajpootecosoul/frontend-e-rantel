...
import type {
  SproutsAgingBucketId,
  SproutsAgingBucketMeta,
  SproutsInventoryDashboard,
} from '@/lib/stores/sprouts/inventory-types';
...
export default function SproutsInventoryAgingCards({
  data,
  loading,
}: {
        ...
        {loading
          ...
          : buckets.map((b) => (
              <BucketCell
                key={b.id}
                label={b.label}
                qty={overview?.[b.id as SproutsAgingBucketId]?.qty ?? 0}
                cost={overview?.[b.id as SproutsAgingBucketId]?.vendorCost ?? 0}
                variant={variant}
              />
            ))}
      </div>

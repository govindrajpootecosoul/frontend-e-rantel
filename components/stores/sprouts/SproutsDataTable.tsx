...
interface SproutsDataTableProps<TRow extends { _id: string }> {
  title: string;
  columns: SproutsTableColumn<TRow>[];
  rows: TRow[];
  loading?: boolean;
  ...
  getCellValue?: (row: TRow, column: SproutsTableColumn<TRow>) => string;
}

export default function SproutsDataTable<TRow extends { _id: string }>({
  title,
  columns,
  rows,
  loading,
